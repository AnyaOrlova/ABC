...
int main() {
  // инициализирует случайные числа
    ...
    */
    ...
    fprintf(fout, "PModel;Task;OpType;Opt;Timer;LNum;AvTime;AbsErr;RelErr;TaskPerf\n");
    for (int i = 0; i < num_test; i++) {
        fprintf(fout, cpuname);
        fprintf(fout, ";");
        switch (i) {
            case 0:
                fprintf(fout, "matrixA * matrixB;OpType: int;Opt: None;");
                break;
            case 1:
                fprintf(fout, "matrixA * matrixB;OpType: double;Opt: None;");
                break;
            case 2:
                fprintf(fout, "matrixA * matrixB;OpType: float;Opt: None;");
                break;
            default:
